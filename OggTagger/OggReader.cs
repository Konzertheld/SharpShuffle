...
namespace JockerSoft
{
    ...
    public class OggReader
    {
        ...
        public Dictionary<string, string> AllTheInformation { get; private set; }

        public OggReader(string FileName)
        {
            AllTheInformation = new Dictionary<string, string>();
                    ...
                    switch (td.GetString(buffer).ToUpper())
                    {
                        case "TITL":
                            ...
                            if (fullTagName(fs) == "TITLE")
                                ...
                                AllTheInformation.Add("title",(this.title = this.readNext(fs)));
                            break;
                        case "VERS":
                            if (fullTagName(fs) == "VERSION")
                                AllTheInformation.Add("version", (this.version = readNext(fs)));
                            break;
                        case "ALBU":
                            if (fullTagName(fs) == "ALBUM")
                                AllTheInformation.Add("album", (this.album = readNext(fs)));
                            break;
                        case "TRAC":
                            if (fullTagName(fs) == "TRACKNUMBER")
                                AllTheInformation.Add("tracknumber", (this.tracknumber = readNext(fs)));
                            break;
                        case "ARTI":
                            if (fullTagName(fs) == "ARTIST")
                                AllTheInformation.Add("artist", (this.artist = readNext(fs)));
                            break;
                        case "PERF":
                            if (fullTagName(fs) == "PERFORMER")
                                AllTheInformation.Add("performer", (this.performer = readNext(fs)));
                            break;
                        case "COPY":
                            if (fullTagName(fs) == "COPYRIGHT")
                                AllTheInformation.Add("copyright", (this.copyright = readNext(fs)));
                            break;
                        case "LICE":
                            if (fullTagName(fs) == "LICENSE")
                                AllTheInformation.Add("license", (this.license = readNext(fs)));
                            break;
                        case "ORGA":
                            if (fullTagName(fs) == "ORGANIZATION")
                                AllTheInformation.Add("organization", (this.organization = readNext(fs)));
                            break;
                        case "DESC":
                            if (fullTagName(fs) == "DESCRIPTION")
                                AllTheInformation.Add("description", (this.description = readNext(fs)));
                            break;
                        case "GENR":
                            if (fullTagName(fs) == "GENRE")
                                AllTheInformation.Add("genre", (this.genre = readNext(fs)));
                            break;
                        case "DATE":
                            fs.Position++;  //avoid to read =
                            AllTheInformation.Add("date", (this.date = readNext(fs)));
                            break;
                        case "LOCA":
                            if (fullTagName(fs) == "LOCATION")
                                AllTheInformation.Add("location", (this.location = readNext(fs)));
                            break;
                        case "CONT":
                            if (fullTagName(fs) == "CONTACT")
                                AllTheInformation.Add("contact", (this.contact = readNext(fs)));
                            break;
                        case "ISCR":
                            fs.Position++;  //avoid to read =
                            AllTheInformation.Add("iscr", (this.iscr = readNext(fs)));
                            break;
                        case "COMM":
                            if (fullTagName(fs) == "COMMENT")
                                try { AllTheInformation.Add("comment", (this.comment = readNext(fs))); }
                                catch { }
                            break;
                        //					default:
